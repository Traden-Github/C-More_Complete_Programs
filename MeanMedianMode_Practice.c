...
int main(void) {
    ...
    printf("TV show audience score calculating system:\n\n"
           "Enter 20 audience scores(0 to 10)for us to\ncalculate "
		   "mean, median, and mode of these scores.\n\n\n");
	int score = 0;
	int scores[SIZE] = {0};
	size_t i1 = 0;
	int count = 0;	
	...
	if(count != SIZE) {
		puts("Oops, your input wasn't correct :C");
...
void mode(int frequency_mode[], const int scores_mode[]) {
	...
	printf("%s","--------\n  Mode\n--------\n\n");
	printf("The mode score(s) is/are %d ", modeValue);
	size_t i6 = 0;
	for(; i6 < 11; ++i6) {
